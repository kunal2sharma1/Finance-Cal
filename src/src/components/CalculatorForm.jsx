...
export default function CalculatorForm({ fields, values, onChange }) {
  return (
    <div className="calc-form">
      {fields.map((field) => {
        // Visual-only: how far along the track the current value sits,
        // used purely to paint the filled portion of the slider. Does not
        // affect the stored value or any calculation.
        const percent =
          ((Number(values[field.name]) - field.min) / (field.max - field.min)) * 100
