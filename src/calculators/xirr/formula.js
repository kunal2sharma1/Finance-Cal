...
export function calculate(inputs) {
  const flows = parseCashFlows(inputs.cashFlows)
    .map((flow) => ({ ...flow, amount: Number(flow.amount) }))
  ...
  if (!hasPositive || !hasNegative) {
    return {
      xirr: 0,
      netCashFlow: Math.round(flows.reduce((sum, flow) => sum + flow.amount, 0)),
      totalInvested: Math.round(Math.abs(flows.filter((flow) => flow.amount < 0).reduce((sum, flow) => sum + flow.amount, 0))),
      totalReceived: Math.round(flows.filter((flow) => flow.amount > 0).reduce((sum, flow) => sum + flow.amount, 0)),
    }
  }

  const rate = solveXIRR(flows)
  const invested = Math.abs(flows.filter((flow) => flow.amount < 0).reduce((sum, flow) => sum + flow.amount, 0))
  const received = flows.filter((flow) => flow.amount > 0).reduce((sum, flow) => sum + flow.amount, 0)
