...
function buildDefaultValues(fields) {
  ...
  for (const field of fields) {
    ...
    } else if (field.type === 'cashflows') {
      values[field.name] = [{ date: '', direction: 'invested', amount: '' }]
...
for (const calculator of calculators) {
  ...
  try {
    ...
    if (!results || typeof results !== 'object') {
      throw new Error('Formula did not return a result object.')
    }

    for (const field of config.resultFields) {
      const value = results[field.name]
      if (value !== undefined && typeof value === 'number' && !Number.isFinite(value)) {
        throw new Error(`Result ${field.name} is not finite.`)
      }
    }
  } catch (error) {
